...
train_data = np.zeros((train_input.shape[0],imsize,imsize,1))
for i in range(train_data.shape[0]):
    train_data[i,:,:,0] = train_output[i,0,:,:]
...
batch_size = 200  ## maybe can be enlarged
noise_dim = 50  ## need to be modified
lr = 0.0002
epsilon = 1e-14
numta = 3e-1
learning_rate = 4e-4
...
img = tf.placeholder(tf.float32,[None,imsize*imsize*channels])
z = tf.placeholder(tf.float32,[None,noise_dim])

fake_image = generator(z)  # [None,imsize*imsize*channels]
...
if train:
    for iteration in range(Iters-Iters_completed+1):
        ...
        if iteration % 100 == 0:
            [W_distance_value,G_loss_value,fake_img_value] = sess.run([W_distance,
                                G_loss,fake_image_re],feed_dict={img:batch_data,z:z_input})
            
            print('Step: {}, W_distance: {:.3f}'.format(iteration,W_distance_value))
...
## real image
fig,ax = plt.subplots(3,2,figsize=(12,15))
real_idx = np.random.choice(K_train_flatten.shape[1],6,replace=False)
