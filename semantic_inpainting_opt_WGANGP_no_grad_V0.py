...
continuous_nonGaussian = False


code_version = 0
repeat_num = np.int32(sys.argv[2])

# 提取训练数据与测试数据  
if data_dir == './datasets_with_KL/':
    output_dir = './semantic_inpainting_WGANGP_no_physics/outputs_with_KL_V{0}/'.format(code_version)
    ...
    if continuous_nonGaussian:
        output_dir = './semantic_inpainting_WGANGP_no_physics/outputs_with_contin_nonGaussian_V{0}/'.format(code_version)
        with h5py.File(data_dir+'continuous_nonGaussian_4000_train.hdf5','r') as f:
            test_input = f['input'][:]
    else:
        output_dir = './semantic_inpainting_WGANGP_no_physics/outputs_with_nonGaussian_V{0}/'.format(code_version)
...
test_data = np.zeros((test_input.shape[0],imsize,imsize,1))
...
with tf.variable_scope('z_optimizer',reuse=tf.AUTO_REUSE): 
    z = tf.get_variable('z',[1,100],initializer=tf.random_normal_initializer())
...
# context_loss = tf.reduce_mean(tf.abs(G_output_k - y_k) * k_mask) + tf.reduce_mean(tf.abs(G_output_h - y_h) * h_mask)
context_loss = tf.reduce_sum(tf.abs(G_output_k - y_k) * k_mask)
loss =  prior_loss + context_loss * 1.

global_step = tf.get_variable('global_step', [], initializer=tf.constant_initializer(0), trainable=False)
learning_rate = tf.train.exponential_decay(1e-1, global_step, decay_step, decay_rate, staircase=False)
# learning_rate = 1e-1
with tf.variable_scope('z_optimizer',reuse=tf.AUTO_REUSE):   
    optimizer = tf.train.AdamOptimizer(learning_rate).minimize(loss,var_list=z,global_step=global_step)
...
## 导入with physics用的point measurements
filling_idx_k = np.int32(np.loadtxt(f'./hard_data/k_idx_{k_obs_percent}.txt'))
# filling_idx_k = np.random.choice(imsize*imsize,np.int32(k_obs_percent*imsize*imsize),replace=False)   ## 随机选取有数据的点
...
p_loss_value_ensem = []
c_loss_value_ensem = []
tf_dict = {k_real_image:real_img_data_k,k_mask:mask_value_k}
...
output_figure_save_dir = output_figure_dir + f'saved_figures_sample_{test_idx}_k_{k_obs_percent}/'
if not os.path.exists(output_figure_save_dir):
    os.makedirs(output_figure_save_dir)

np.savetxt(output_figure_save_dir + f'{repeat_num}_generated_k.txt',blend_k.ravel())
...
ax.set_ylim(0,Ly)
plt.tight_layout()
plt.savefig(output_figure_save_dir + f'{repeat_num}_point_obs.jpg')
...
np.savetxt(output_figure_save_dir + f'{repeat_num}_k_corrcoef.txt',np.array(coef).reshape(-1,1))
...
np.savetxt(output_figure_save_dir + f'{repeat_num}_k_rmse.txt',k_rmse)